...
$errormsg = "";
		...
		{	
			...
			//Data Cleansing
			$formfield['ffticketOption'] = trim($_POST['ticketOption']);
	...
	//check for empty fields
		...
		if(empty($formfield['ffticketOption'])){$errormsg .= "<p>Your pickup choice is empty.</p>";}
	...
	//check for errors
			...
			else
			{
				try
				{
					...
					$sqlinsert = 'update tickets set ticketOption = :bvticketOption,
								  ticketDate = :bvticketdate
								  where ticketNum = :bvticketNum';
					$stmtinsert = $db->prepare($sqlinsert);
					$stmtinsert->bindvalue(':bvticketOption', $formfield['ffticketOption']);
	...
	if ($showform == 1  && $visible == 1 && $_SESSION['cartloginpermit'] == 1)
	{
	?>
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" name="theform">
			<fieldset><legend>Ticket Information</legend>
					...
					<tr>
						<th>Pick a Delivery Type:</th>
						<td><input type="radio" name="ticketOption" id="ticketOption" 
									value=1 <?php if ($formfield['ffticketOption'] == 1) { echo ' checked';} ?> />
							<label for="pickup">Pickup</label>
							<input type="radio" name="ticketOption" id="ticketOption" 
									value=2 <?php if ($formfield['ffticketOption'] == 2) { echo ' checked';} ?> />
									<label for="delivery">Delivery</label>
							</td>
					</tr>
					<tr>
