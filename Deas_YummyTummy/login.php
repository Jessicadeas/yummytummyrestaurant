	...
	else
	{
		try
		{
			$sql = 'SELECT * FROM userinfo WHERE dbemail = :bvemail
					AND dbuserpermit = :bvuserpermit';
			$s = $db->prepare($sql);
			$s->bindValue(':bvemail', $formfield['ffemail']);
			$s->bindValue(':bvuserpermit', 5);
			$s->execute();
			$count = $s->rowCount();
		}
		...
		else 
		{
			...
			{
				$_SESSION['cartloginid']= $row['this'];
                $_SESSION['cartloginname'] = $row['dbfullname'];
				$_SESSION['cartloginpermit'] = $row['dbuserpermit'];
				$showform = 0;
				echo "<br>";
                echo "Logged In Successfully";
				echo "<br><br>";
				echo '<a href = "selectcart.php">Continue</a>';
				echo "<br>";
			} 
			else
			{
